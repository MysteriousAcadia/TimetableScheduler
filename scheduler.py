...
from util import load_data, show_timetable, set_up, show_statistics, write_solution_to_file
...
def initial_population(data, matrix, free, filled, groups_empty_space,
        ...
        while True:
            ...
            if found:
                for group_index in classs.groups:
                    ...
                    insert_order(subjects_order, classs.subject, group_index,
                                 classs.type, start_time)
                ...
                for i in range(int(classs.duration)):
                    ...
                    free.remove(
                        (i + start_time, start_field[1]))  # remove from free
...
def exchange_two(matrix, filled, ind1, ind2):
    ...
    for i in range(len(fields1)):
        ...
        matrix[fields1[i][0]][fields1[i][1]] = matrix[fields2[i][0]][fields2[i]
                                                                     [1]]
...
def simulated_hardening(matrix, data, free, filled, groups_empty_space,
    ...
    for i in range(iter_count):
        ...
        for j in range(len(data.classes) // 4):
            ...
            mutate_ideal_spot(matrix, data, index_class, free, filled,
                              groups_empty_space, teachers_empty_space,
                              subjects_order)
